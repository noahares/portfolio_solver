...
use ndarray::Shape;
use polars::{prelude::*, lazy::dsl::{as_struct, GetOutput, Expr}};
...
impl Data {
    pub fn new(path: &str, k: u32) -> Result<Self> {
        let schema = Schema::from(
            vec![
            Field::new("km1", DataType::Float64)
            ].into_iter());
        let kahypar_columns = vec!["algorithm".to_string(), "graph".into(), "k".into(), "imbalance".into(), "km1".into(), "partitionTime".into()];
        let target_columns = vec!["algorithm", "instance", "k", "feasibility_score", "quality", "time"];
        ...
        let df = CsvReader::from_path(path)?
            ...
            .sort_by_exprs(sort_order.iter().map(|o| col(o)).collect::<Vec<Expr>>(), vec![false; 3], false)
            .collect()?;

        let instances = ndarray::Array1::from_iter(df.column("instance")?.unique()?.utf8()?.into_no_null_iter().map(|s| s.to_string()));
        let algorithms = ndarray::Array1::from_iter(df.column("algorithm")?.unique()?.utf8()?.into_no_null_iter().map(|s| s.to_string()));
        let instance_multiplier = instance_fields.iter().map(|field| Ok::<usize, PolarsError>(df.column(field)?.unique()?.len())).fold_ok(1, Mul::mul)?;
        let num_instances = instances.len() * instance_multiplier;
        let num_algorithms = algorithms.len();
        let best_per_instance = best_per_instance(df.clone().lazy(), &instance_fields)?;
        let stats = stats(df.clone().lazy(), k, &instance_fields, Shape::from(ndarray::Dim([num_instances, num_algorithms, k as usize])))?;
        Ok(Self {
            df,
            instances,
            algorithms,
...
fn best_per_instance(df: LazyFrame, instance_fields: &[&str]) -> Result<ndarray::Array1<f64>> {
    let best_per_instance = df.groupby([&["instance"], instance_fields].concat())
        .agg([min("quality").alias("best")])
        .collect()?;
    Ok(best_per_instance.column("best")?.f64()?.to_ndarray()?.to_owned())
}

fn stats(df: LazyFrame, sample_size: u32, instance_fields: &[&str], shape: Shape<ndarray::Dim<[usize; 3]>>) -> Result<ndarray::Array3<f64>> {
    ...
    let stats = df
        ...
        .select([
                ...
                col("instance"),
                col("sample_size"),
                as_struct(&[col("mean_quality"), col("std_quality"), col("sample_size")])
                .apply(
                    |s| {
                        let data = s.struct_()?;
        ...
        .collect()?;
    let stats_array: ndarray::Array3<f64> = ndarray::Array3::<f64>::from_shape_vec(shape, stats.column("e_min")?.f64()?.into_no_null_iter().collect::<Vec<f64>>())?;
    Ok(stats_array)
}
